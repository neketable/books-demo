...
server.post('/token', function (req, res) {
  ...
  if (user) {
    ...
    res.status(401).json(getError('Login', 'Неверный логин или пароль', 401, null));
  }
});
...
server.use((req, res, next) => {
  ...
  if (getBaseRoute(req) === 'users' && req.method === 'POST' && !valid) {
    res.status(422).json(getError('email', 'Почта уже используется', 422, '/data/attributes/email'));
  }
